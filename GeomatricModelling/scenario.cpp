...
void Scenario::initializeScenario() {
  ...
  // Surface
  //auto curve = new firstCurve<float>;
  _firstcurve = std::make_shared<firstCurve<float>>();

  _firstcurve->toggleDefaultVisualizer();
  _firstcurve->setColor(GMlib::GMcolor::Red);
  _firstcurve->replot(100,1);
 // scene()->insert(_firstcurve.get());



  _gerbs = std::make_shared<gerbsCurve<float>>(_firstcurve,10);
     _gerbs->replot(100,3);
    _gerbs->toggleDefaultVisualizer();
    _gerbs->translateGlobal(GMlib::Vector<float,3>(2.0f,1.0f,1.0f));
   _gerbs->setColor(GMlib::GMcolor::Blue);

   scene()->insert(_gerbs.get());
